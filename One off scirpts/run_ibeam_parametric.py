...
try:
    ...
    if args.load_distribution == "uniform":
        ...
        # Linearly varying pressure along y-axis, scaled to keep total force constant
    ...
    if args.load_type == "bending_y":
        ...
        # Torsion uses a uniform tangential traction distribution, overriding the linear one
    ...
    warped = grid.warp_by_vector("u", factor=200)
    plotter_def = pyvista.Plotter(off_screen=True, window_size=[800, 600])
    plotter_def.add_mesh(grid, style='wireframe', color='gray', opacity=0.5)
    plotter_def.add_mesh(warped, show_edges=False, scalars=np.linalg.norm(grid['u'], axis=1))
